...
src/
  ...
  components/
    ...
    PlatbyView.tsx       # Platby „autorizační kalkulačka": souhrn Banka+Hotovost (zeleně/červeně) + drill-down Provoz→Kategorie→Faktura + Uhradit ihned + autorizace heslem (viz platbyModulData.ts)
...
### Session log
- **v3 s41**: **Faktury — filtr „Stavy" (jednotný multiselect) + DUZP v „Podle" (zápis 24. 7. 2026)**. Práce ve `FakturyView.tsx` + `FakturyTable.tsx`. Dle poznámky kolegyň: ① **Pole „Stav" přejmenováno na „Stavy"** a rozšířeno na **jednotný zaškrtávací multiselect se 17 možnostmi** napříč čtyřmi skupinami — **úhrada** (Neuhrazené / Uhrazené / Uhrazené zápočtem / Uhrazené zálohovou fakturou), **workflow** (Schválené / V bance / V bance neuhrazené / Pozastavené / Po splatnosti), **způsob platby** (Hotově / Kartou / Stravenkami / Bankovním převodem), **párování s DL** (Čeká na spárování / Nespárované / Spárované / Duplicitní DL). Nový typ `StavyKey` + `STAVY_OPTIONS` (`FakturyView`) + predikát `matchStavy(f, key, effStav)` (`FakturyTable`) mapující každý klíč na skutečné filtrování (úhrada dle `getEffektivniStav`, workflow stav, `getZpusobUhrady`, `getMatchingData`). Vybrané klíče = **OR** (faktura projde, pokud odpovídá kterémukoli). Nový prop `stavyFilters?: Set<StavyKey>`. `StavCheckDropdown` zobecněn na generický `<T extends string>`. Kompaktní upozornění nad tabulkou přepojena na `setStavyFilters` (Neuhrazené dobropisy/zálohové → `'neuhrazene'`, Zálohové bez finální → `'uhrazene'`, Pozastavené → `'pozastavene'`). Odstraněn nepoužitý `STAV_UHRADY_OPTIONS` + `stavUhrady` state. ② **„Podle" má nově 3 možnosti** — Datum vystavení · Datum splatnosti · **Datum DUZP** (`datumPodle` rozšířeno o `'duzp'`, filtrace `f.duzp ?? f.datum`). ③ **Rozšířený filtr (modal) zrušen** — původně přidán dle „tipu" (obrázek fakturačního nástroje s ~18 poli), ale kolegyně potvrdily, že stačí „Stavy" + DUZP v „Podle". Odstraněn modal, tlačítko „Rozšířené filtry", stavy (`advOpen`/`vystaveniOd/Do`/`duzpOd/Do`/`zaplaceno`), props ve `FakturyTable` i CSS `.faktury-adv-*`. Výsledný filtr: **Hledat · Rychlý výběr · Stavy · Seřadit** (řádek 1) + **Od · Do · Podle · Obsahuje náklad · Částka od/do** (řádek 2). Build OK. **Parkoviště (příště):** nákladové položky **vydaných** faktur do editoru „Vystavit fakturu" (Vouchery / Nabití konta / Nájem / Voda / Elektřina / Plyn / Trasa peněz faktura / Služby / Zpětný bonus / Prodej vozu / Projektová dokumentace / Catering / Hitachi) — výběr při psaní vydané faktury.
- **v3 s40**: **Platby — nový modul „autorizační kalkulačka" (zápis + nákres 22. 7. 2026)**. Kompletní přestavba `PlatbyView` (starý platební dashboard nahrazen) na základě zápisu z porady + ručního nákresu. Nový mock `platbyModulData.ts`. **Koncept:** celofiremní pohled — majitel vidí nahoře dvě souhrnné tabulky (BANKA vlevo / HOTOVOST vpravo), obě svítí zeleně (dostatek) / červeně (nedostatek). Dole provozy s předvolenými budoucími platbami. Když je vše zelené → jedním klikem + heslem autorizuje všechny platby do termínu „Do:". Když je někde nedostatek → odklikne provoz/kategorii/fakturu, tabulky se živě přepočítají, odkliknuté zůstanou na příště. **Vrstvy:** ① **Kostra + celofiremní pohled** — nadpis Platby + badge „Celá firma"; výběr provozovny v Topbaru **zašedlý/disabled** na sekci `platby` (`selectedSection === 'platby'`); pole **„Platit do"** (default +7 dní = 2026-04-24, nativní kalendář) filtruje faktury dle splatnosti (odchod 2 dny předem řeší pozadí). ② **Dvě souhrnné tabulky** (`.platby-souhrn`, zelený/červený inset okraj): **Banka** = Aktuální stav účtů − Trvalé příkazy − Úvěry − Daně − Poplatky − Platby (vybrané faktury) − Výběr hotovosti + Predikce (karty na cestě) → **Budoucí zůstatek** (zeleně/červeně). **Hotovost** = Trezory − Kasař (rezerva) = Disponibilní hotovost − Plánované výplaty hotově; při schodku **„Nutný výběr z banky"** + per-provoz rozpad **„kde vybrat"** (promítne se do banky jako výběr hotovosti). ③ **Drill-down Provoz → Kategorie → Faktura** (`PLATBY_PROVOZY` → `kategorie[]` → `faktury[]`): 7 kategorií (`KATEGORIE_META`: faktury/investice/dph/energie/výplaty na účet/odvody/výplaty hotově). Tri-state checkboxy na každé úrovni (`Check` komponenta, indeterminate), odškrtnutá faktura = nechat na příště. `vybraneBreakdown()` rozdělí vybrané na platby / daně (DPH+odvody) / výplaty hotově. ④ **Uhradit ihned** na úrovni faktury (§4) — přepne splatnost na „dnes", vynutí výběr. ⑤ **Detail faktury = sdílený `FakturySidePanel`** — na přání kolegyň „úplně stejný detail napříč všemi podstránkami, jen jiné CTA". `FakturySidePanel` rozšířen o props **`hideWorkflow`** (skryje schvalovací akce) + **`ctaSlot`** (vlastní CTA blok). Platby konstruují `FakturaPlatby` z `PlatbaFaktura` (`toFakturaPlatby` + mapování kategorií `KAT_TO_FAKTURA`) → stejné záložky Detail/Párování/Komunikace/Historie + náhled + přílohy, jen CTA = **Uhradit ihned / Nechat na příště**. Offcanvas `.platby-overlay-panel`. ⑥ **Autorizace heslem** (§4) — modal `.platby-modal` s heslem (test **`2026`** všude); po potvrzení děkovací obrazovka „Právě jste odeslal platby za tyto provozy" + badge provozů + částka. Tlačítko Autorizovat aktivní jen když budoucí zůstatek ≥ 0. **Default mock = zelený happy-path** (budoucí zůstatek ~1,77 mil.). **Zbývá k doladění** (příště): vizuál blíž nákresu (zatím „jen tabulka"), rozklikávací seznam účtů v souhrnu, detailnější hotovostní workflow (výběry na výplaty per provoz jako akce), demonstrace červeného stavu, TP dle „Do:". Build OK ~940 KB JS bundle.
- **v3 s39**: **Faktury přijaté — zjednodušení filtru na 2 řádky + doladění kompletních faktur / detailu (zápis 22. 7. 2026)**. Navazuje na s38, dokončuje sekci Přijaté. Práce v `FakturyView.tsx` + `FakturyTable.tsx` + `custom.css`. **(A) Zjednodušení filtru** (dle přání kolegyň „min. klikání"): filtr **z 3 řádků na 2**. ① **Stav jako zaškrtávací dropdown** — nová komponenta `StavCheckDropdown` (button „Všechny stavy / Vybráno: N" + rozbalovací seznam checkboxů, click-outside zavření, „Zrušit výběr (N)"). Nahradila 9 chip-tlačítek roztažených přes celou šířku. ② **Řádek 1** (vždy): Hledat · Rychlý výběr (období) · Stav (dropdown) · Seřadit. **Řádek 2** (vždy, bez CTA): Od · Do · Podle · Obsahuje náklad · Částka od · Částka do. _(Nejdřív byl řádek 2 za tlačítkem „Další filtry" (collapse), pak na přání odbaleno natrvalo a tlačítko + `showMoreFilters` state odstraněny.)_ ③ **Sloučeno „Seřadit podle" + „Seřadit"** do jednoho selectu (value `sortBy:sortDir` — Splatnost nejnovější/nejstarší · Částka ↑↓ · Dodavatel A→Z · Podle stavu). ④ **Zrušeno mrtvé pole „Vyhledat podle"** (`searchBy` — state se nikam nepředával, tabulka stejně vždy hledala jen dodavatele). Hledání v `FakturyTable` rozšířeno na **dodavatel + číslo faktury + VS** (`hay = dodavatel + cislo + getVS`). **(B) Doladění (4 body ze zpětné vazby)**: ① **`isComplete` rozšířeno** — kompletní = zaplaceno (uhrazená/zaplacená) **A** párování bez otevřeného problému. „Vyřešené" párování = spárováno se Septimem **NEBO bez DL** (nepáruje se, schvaluje ručně — energie/nájmy/služby). Otevřené problémy (`MATCHING_UNRESOLVED` = nesedi-dl/duplikat/castecne-sparovana/ceka-na-sparovani) brání kompletnosti. Zelených faktur 1→3 (přibyl E.ON energie). ② **Legenda** pod tabulkou doplněna o `zelená = kompletní (nic k řešení)`. ③ **Tab lišta detailu přišpendlená** hned pod hlavičkou — výška hlavičky se měří (`overlayPanelRef` → CSS var `--fk-detail-hdr-h`) a `.faktury-panel-tabs` má `position: sticky; top: var(--fk-detail-hdr-h)`. Hlavička (identita) i záložky drží při scrollu, obsah roluje pod nimi. ④ **Responzivní šířky overlaye** — desktop **600 px**, tablet (≤1200) **560 px**, mobil (≤768) **fullscreen** + zvětšený `.btn-close` (snadné zavření). Build OK. **Přijaté faktury tímto hotové.** Další na řadě: modul **Platby** (nový, dlouhé ladění). Zbývající sekce: Daně, Vydané, Banka, Úvěry, Trvalé příkazy.
